...
- **`@web-media/definition`:** This package defines the metadata for each
  resource file. Developers can manually write definitions for each file, but it
  is recommended to use Web Media Foundation Studio to generate them for a more
  user-friendly interface and ease of management for larger sets of files.

- **`@web-media/client-sdk`:** If using Web Media Foundation Studio to generate
  configurations, this package can utilize the output and handle data fetching
  and decoding tasks.

- **`@web-media/act-player`:** If using `@web-media/client-sdk`, the player is
  ...
  `@web-media/core-manager` for resource management.

- **`@web-media/core-manager`:** This layer is responsible for managing the
  selection of resources.

### Resource selection

If the developer required a resource group, like a texture. The resource will be
selected with a weighted score. Web Media Foundation will pick the resource with the
maximum score as the result resource file.

Such tasks is finished by the [`@web-media/smart-resource`](/api/smart-resource/function/calculateResourceScore)
...
- **Environmental variable configurations: ** This is the detailed config of
  users' devices. If you are using the API provided by `@web-media/core-manager`,
  these variables would be specified by the [`EnvVariableManager`](https://github.com/Web-Media-Foundation/infrastructure/blob/a391b0dfbc42a8ccffcec6404dc3e86bb019f12f/packages/core-manager/src/manager/envVariable/EnvVariableManager.ts)
  ...
  - **`screen`:** The screen size is [small, medium or large](https://github.com/Web-Media-Foundation/infrastructure/blob/a391b0dfbc42a8ccffcec6404dc3e86bb019f12f/packages/core-manager/src/manager/envVariable/utils/screenSize.ts#L1).

## Configuration Generation

If developers decide to manage their resource files with the Web Media Foundation Studio,
the configuration will be managed automatically. By importing the resource will
be processed by various extensions:

- **`@web-media/extension-av`:** Splits audio and video files into different
  channels for the video playback system provided by the Web Media Foundation. This
  improves consistency across all major platforms. The audio file will be
  converted to fix any possible compatibility issues.
- **`@web-media/atlas`:** Analyzes the dimensions of textures and packs them into
  an atlas texture to save memory and improve the stability of the application
  on mobile platforms.
- **`@web-media/extension-audio-backends`:** Provides options on the interface
...
- **`@web-media/extension-crowdin`: ** Provides scripts for synchronizing
  translation files from the Crowdin platform. These files can be used for
  internationalization purposes.

:::info
Although there is currently no way for users of the Web Media Foundation Studio to add
third-party extensions, developers can create a pull request to the code
repository and discuss their desired extension with the Web Media Foundation team.
...
The official implementation of the Video component in the Web Media Foundation Studio
...
component provided by the `@web-media/ap-core`, here is an example:

```ts
import { SmartSprite } from '@web-media/ap-core';
...
Please checkout the [official implementation](https://github.com/Web-Media-Foundation/infrastructure/blob/feb76e968128289ccde3782dea390e7decc766a1/packages/act-player/src/components/Stage/Video.tsx#L228-L299)
for more detail.
